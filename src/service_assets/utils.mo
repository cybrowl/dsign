...
module {
    ...
    public func generate_asset_url(args : GenerateAssetUrlArgs) : Text {
        var url = Text.join("", (["https://", args.canister_id, ".raw.ic0.app", "/asset/", args.asset_id].vals()));

        if (args.is_prod == false) {
            url:= Text.join("", (["http://localhost:8000/asset/", args.asset_id, "?canisterId=", args.canister_id].vals()));
        };
